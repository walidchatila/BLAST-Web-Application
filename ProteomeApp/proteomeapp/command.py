...
# symbols which are imported by "from proteomeapp.command import *"
...
from proteomeapp.release import version
    ...
    if args:
        ...
        try:
            configfile = pkg_resources.resource_filename(
              pkg_resources.Requirement.parse("proteomeapp"),
                "config/default.cfg")
        except pkg_resources.DistributionNotFound:
            raise ConfigurationError("Could not find default configuration.")

    turbogears.update_config(configfile=configfile,
        modulename="proteomeapp.config")
...
def bootstrap():
    ...
    By default this runs the function 'proteomeapp.model.bootstrap_model', which
    ...
    easy_install which will run this function:
        ...
        'bootstrap-proteomeapp = proteomeapp.command:bootstrap',
    ...
    optparser = optparse.OptionParser(usage="%prog [options] [config-file]",
        ...
        "config-file.", version="ProteomeApp %s" % version)
    ...
    _read_config(args)
    from proteomeapp.model import bootstrap_model
...
def start():
    ...
    from proteomeapp.controllers import Root
